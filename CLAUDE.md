...
│   └── lib/api.js             # fetch() wrapper for all API calls (uses relative /api base)
│
...
- `enableOfflineQueue: false` — commands fail immediately when disconnected; Sync Worker waits for `redis.status === 'ready'` before issuing `xgroup` to avoid startup crash
...
| `docs/required_info.md` | Stack, ports, credentials, schema summary |
| `docs/subagent.md` | Subagent decomposition and agent contracts |
